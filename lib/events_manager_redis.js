...
module.exports.store = function(event, callback){
  ...
  //another approach
  client.hset(event.key, 'description', event.description);
  client.hset(event.key, 'startDateTime', event.startDateTime.getTime());
  client.hset(event.key, 'moderatorPass', event.moderatorPass);
  client.hset(event.key, 'status', event.status);
  callback(null, null);
  //TODO - [comments]
  //TODO - [tags]
...
//  });
  ...
  client.hgetall(key, function(err, reply){
    console.log('em.getByKey reply:%s', JSON.stringify(reply));
    callback(reply);
  });
